...
def load_dataset(args):
    ...
    _, val_dataset = dataset_split(data_tensor, val_split=0.1)
    val_dataset = TensorDataset(val_dataset, val_dataset)
    dataloader = DataLoader( 
        dataset=val_dataset, 
        batch_size=args.mini_hessian_batch_size, 
        shuffle=False
    )
    return dataloader
...
def main(args):
    ...
    plt.grid()
    plt.savefig(os.environ["HAWQ_JET_TAGGING"]+'/results/hessian/ad_trace.png')
    plt.yscale('log')
    plt.savefig(os.environ["HAWQ_JET_TAGGING"]+'/results/hessian/ad_trace-log.png')
    plt.close()
...
# python hessian.py --resume /data/jcampos/hawq-jet-tagging/checkpoints/ad/ToyCar/10.31.2023-21.52.54/model_best.ckpt --data /data/jcampos/hawq-jet-tagging/training/anomaly_detection/train_time_data/train_data_inputs_64_frames_5_hops_512_fft_1024_mels_128_power_2.0.npy 
